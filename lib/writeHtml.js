  ...
  write: function(emps) {
    ...
    async function writeToFile(text) {
      try {
        ...
        await writeFileAsync("./Assets/output/output.html", text);
      }  //end of try block
      // handle error
      catch (err) {
        console.log(err);
    ...
    const buildBegHtml = (code) => {
      ...
      code = code + '<meta name="viewport" content="width=device-width, initial-scale=1.0">\n';       
      code = code + '<meta http-equiv="X-UA-Compatible" content="ie=edge">\n';
      code = code + '<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css"        integrity="sha384-HSMxcRTRxnN+Bdg0JdbxYKrThecOKuH5zCYotlSAcp1+c8xmyTe9GYg1l9a69psu" crossorigin="anonymous">\n';
      ...
      code = code + '<container>\n';
      code = code + '<div class="row row-margin">\n';

      return code;
    };  // end of buildBegHtml function

    const buildCard = (code, employee) => {
      let colEl = "<div class='col-sm-5 col-md-4 col-lg-3 border-info col-margin'>";
      code = code + colEl + "\n";
      ...
      switch (employee.title) {
        case "Manager":
          listEl = listEl + `<li>Office Number:  ${employee.officeNumber}</li>` + "\n";
          break;
        case "Engineer":
